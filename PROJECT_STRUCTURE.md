...
### 项目bugList
1. 财务支出页面，新增支出后，无数据展示，首页今日概览也无今日支出数据；
2. 任务管理页面新增任务只填写任务标题和任务描述，可以点击保存，但是保存后无法查到改数据，首页概览也不显示该任务条数。新增任务时填写了任务时间，点击保存无响应；
3. 日记页面右下角 新增日记按钮，有重复图标；

### 解决方案
1. 财务支出页面，新增支出后，在财务页面能看到今日支出/收入详情，可以以日/周/月/年为单位查看收支详情；同时将当日收支同步到首页概览；
2. 任务管理页面新增任务，标题必填、描述选填，时间必填，后点击保存按钮，可以在管理页面查到今日/周/月的任务；同时首页概览显示今日待办事项数量；
